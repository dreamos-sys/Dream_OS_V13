...
(function() {
    ...
    function addOptimisticRow(formData) {
        ...
        row.innerHTML = `
            ...
            <td class="px-2 py-2 text-yellow-600">⏳ pending (optimistic)</td>
        `;
        tbody.prepend(row); // tampil paling atas
        return tempId;
    }
    ...
    document.getElementById('bookingForm').addEventListener('submit', async (e) => {
        ...
        // Data untuk optimistic
        const formDataOptimistic = {
            nama,
            sarana,
            jam_mulai: jamM,
            jam_selesai: jamS,
        ...
        // Tambah baris optimistic
        const tempId = addOptimisticRow(formDataOptimistic);

        // Step 2: Cek Double Booking (async)
        ...
        if (error) {
            ...
            // Hapus baris optimistic, lalu reload riwayat dari database
            removeOptimisticRow(tempId);
            resDiv.innerHTML = `<span class="text-green-500 animate-bounce">BERHASIL! ✅ Menunggu Approval...</span>`;
            e.target.reset();
            await loadHistory(); // refresh dengan data asli
    ...
    // Tambahkan refresh manual (opsional)

...
(function() {
    ...
    // ========== CEK SUPABASE (VALIDASI LEBIH KETAT) ==========
    if (!window.supabase || typeof window.supabase.from !== 'function') {
        log('❌ window.supabase tidak valid! Pastikan inisialisasi benar.');
        debugDiv.innerHTML += '<div style="color:red; margin-top:8px;">⚠️ SUPABASE TIDAK VALID – CEK INDEX.HTML</div>';
        return;
    }
    ...
    window.loadFiles = async function() {
        ...
        try {
            // Cek apakah supabase.storage tersedia
            if (!supabase.storage) {
                throw new Error('supabase.storage undefined – mungkin API key tidak memiliki akses storage?');
            }

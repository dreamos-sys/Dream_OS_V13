...
(function() {
    ...
    // ========== DEBUG CONSOLE DENGAN TOMBOL TOGGLE ==========
    ...
    document.getElementById('toggle-debug').addEventListener('click', function(e) {
        e.stopPropagation();
        const content = debugDiv.querySelector('div:last-child');
    ...
    // Close button
    document.getElementById('close-debug').addEventListener('click', function(e) {
        e.stopPropagation();
...
if (!supabase) {
    ...
    // ========== FUNGSI DENGAN PENGECEKAN SUPABASE ==========

    async function loadAnalytics() {
        if (!supabase) return log('Supabase tidak tersedia, lewati loadAnalytics');
    ...
    async function loadRuangKerja() {
        if (!supabase) return log('Supabase tidak tersedia, lewati loadRuangKerja');
    ...
    async function loadAllBookings() {
        if (!supabase) return log('Supabase tidak tersedia, lewati loadAllBookings');
    ...
    async function initCalendar() {
        if (!supabase) return log('Supabase tidak tersedia, lewati initCalendar');
    ...
    window.viewBookingDetail = async (id) => {
        if (!supabase) {
            alert('Database tidak tersedia, tidak dapat melihat detail');
            return;
        }
    ...
    async function updateStats() {
        if (!supabase) return log('Supabase tidak tersedia, lewati updateStats');
        try {
    ...
    async function loadApprovals() {
        if (!supabase) return log('Supabase tidak tersedia, lewati loadApprovals');
    ...
    async function loadDanaList() {
        if (!supabase) return log('Supabase tidak tersedia, lewati loadDanaList');
    ...
    async function loadSpjList() {
        if (!supabase) return log('Supabase tidak tersedia, lewati loadSpjList');
    ...
    async function loadSlidePreviews() {
        if (!supabase) return log('Supabase tidak tersedia, lewati loadSlidePreviews');
    ...
    async function performSearch(query) {
        if (!supabase) {
            searchResults.innerHTML = '<div class="p-3 text-red-400">Database tidak tersedia</div>';
    ...
    window.loadFiles = async function() {
        if (!supabase || !supabase.storage) {
            log('❌ Supabase storage tidak tersedia');
            const container = document.getElementById('file-list');
            if (container) container.innerHTML = '<div class="text-center text-red-500 py-8 col-span-full">Storage tidak tersedia</div>';
            return;
        }
        const container = document.getElementById('file-list');
        if (!container) {
